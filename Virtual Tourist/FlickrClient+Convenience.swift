...
//					guard let maxPageNumber = result[FlickrClient.ResponseKeys.MaxPage] as? Int else {
//						let userInfo = ["NSUnderlyingErrorKey": "Could not access number of pages in response"]
//						let error = NSError(domain: "searchPhotosWithLocation", code: 10, userInfo: userInfo)
//						completionHandlerForSearchPhotos(result: nil, pages: nil, error: error)
//						return
//					}
